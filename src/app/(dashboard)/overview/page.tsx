        ...
        {MODULES.map(({ href, navKey, descKey, icon: Icon }) => (
          ...
          >
            ...
            <span className="space-y-1 pe-5">
              <span className="block text-sm font-semibold tracking-tight">{t(`nav.${navKey}`)}</span>
              <span className="block text-xs leading-relaxed text-muted-foreground">{t(`dashboard.modules.${descKey}`)}</span>
            </span>
          </Link>
        ))}
      </div>
